...
def _clause_cards_html(checks):
    ...
    for c in checks:
        ...
        if c.get("evidence"):
            evidence_html = (
                f'<div class="evidence-snippet">'
                f'&ldquo;{c["evidence"]}&rdquo;'
                f'</div>'
            )
        elif c["pass"] is None:
            evidence_html = (
                '<div class="evidence-snippet placeholder">'
                'Check not yet implemented / awaiting calibration data'
                '</div>'
            )
        else:
            evidence_html = (
        ...
        secondary_html = "".join(
            f"<div>{b}</div>" for b in secondary_bits
        )

        cards.append(
            f"""
            <div class="clause-card{muted_class}"
...
def _build_html(scan: dict, base_dir: str) -> str:
    ...
    if image_url:
        ...
        # Convert the stored relative path into an absolute path.
        image_path = os.path.abspath(
            os.path.join(base_dir, image_url)
        )

        # WeasyPrint works more reliably with a proper file:// URI
        # instead of a raw Windows path such as C:\folder\image.jpg.
        ...
        image_html = (
            '<div class="no-photo">'
            'No product photo available for this scan'
            '</div>'
        )
    ...
    if result.get("needs_human_review"):
        ...
        review_html = (
            ...
            '⚠ Flagged for human review — one or more checks '
            'below have low confidence'
            '</div>'
        )
    ...
    # CLAUSE RESULTS
    # ---------------------------------------------------------
    clause_cards = _clause_cards_html(
        result["checks"]
    )
    ...
    <body>
        ...
        <div class="header-band">
            ...
            <div class="header-meta">
                ...
                Generated:
                {_format_timestamp(scan['timestamp'])}
        ...
        <div class="evidence-panel">
            ...
            <div class="meta-block">
                ...
                <table>
                    ...
                    <tr>
                        <td class="meta-key">Product ID</td>
                        <td>{scan['id']}</td>
                    </tr>

                    <tr>
                        <td class="meta-key">Product Category</td>
                        <td>{scan.get('category') or 'N/A'}</td>
                    </tr>

                    <tr>
                        <td class="meta-key">Region</td>
                        <td>{scan.get('region') or 'N/A'}</td>
                    </tr>

                    <tr>
                        <td class="meta-key">Scanned By</td>
                        <td>{scan.get('user_role') or 'N/A'}</td>
                    </tr>

                    <tr>
                        <td class="meta-key">Consent Given</td>
                        <td>{scan.get('consent_flag')}</td>
                    </tr>
        ...
        <div class="summary-line">
            Font-size check excluded from the passed-count above
            when not evaluated.
        </div>
        ...
        <div class="footer">
            ...
            This is a system-generated preliminary compliance check
            produced by an AI-based OCR and rule-matching pipeline.
            It is intended to assist review and prioritization,
...
def _csv_rows(scan):
    ...
    rows = [
        ...
        [
            "Passed",
            f"{result['passed_count']}/{result['total_checks']}"
        ],

        [
            "Image URL",
            scan.get("image_url") or "N/A"
        ],
